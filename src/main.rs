...
fn chunk_bytes_cdc(
    ...
    for (i, &byte) in data.iter().enumerate() {
        ...
        if boundary_pattern_hit || forced_cut {
            ...
            chunks.push(data[chunk_start_index..=i].to_vec());
    ...
    if chunk_start_index < data.len() {
        chunks.push(data[chunk_start_index..].to_vec());
    }
...
fn main() {
    let path = env::args()
        .nth(1)
        .unwrap_or_else(|| return "/tmp/test.txt".to_string());
    ...
    let target_avg_chunk_size = 8;
    let max_chunk_size = 16;

    let chunks = chunk_bytes_cdc(&data, min_chunk_size, target_avg_chunk_size, max_chunk_size);
